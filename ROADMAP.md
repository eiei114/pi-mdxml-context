...
| Latest published on npm | `0.1.15` (`0.1.16` is staged; publishing remains human-owned via `publish.yml`) |
...
- ~~Resolve the npm publish gap (0.1.3–0.1.5)~~ **Done** — 0.1.3–0.1.15 are on npm; 0.1.16 remains staged pending the human-owned publish step.
...
- [ ] Release pipeline publishes every validated version (latest published `0.1.15`; `0.1.16` pending human-owned publish).
...
— registry latest matches the published package (`0.1.3`–`0.1.15` on npm via Trusted
